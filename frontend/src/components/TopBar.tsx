...
import { Box, AppBar, Toolbar, Typography, Avatar, Menu, MenuItem, IconButton, Button, useTheme, useMediaQuery } from "@mui/material";
      ...
      <AppBar position="static" sx={{ backgroundColor: theme.palette.background.paper, boxShadow: 1 }}>
        <Toolbar>
          <Box sx={{ display: "flex", alignItems: "center" }}>
            ...
            <Typography variant="h6" sx={{ color: theme.palette.text.primary, fontSize: { xs: "1rem", sm: "1.25rem", md: "1.5rem" } }}>
              Music App
            </Typography>
          </Box>
          <Box sx={{ flexGrow: 1 }} />
          <Box sx={{ display: "flex", alignItems: "center", gap: { xs: "8px", sm: "16px" } }}>
            {!isMobile && isAdmin && (
              <Button variant="outlined" sx={{ color: theme.palette.primary.main, borderColor: theme.palette.primary.main, fontSize: { xs: "0.75rem", sm: "1rem" } }}>
                Admin Dashboard
              </Button>
            )}
            {user ? (
